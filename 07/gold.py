...
def emptying(directory, constanta):
    if directory.size >= constanta:
        option = directory.size
    else:
        option = 7000000000
    for x in directory.children:
        option = min(option,emptying(x,constanta))
    return option
print(emptying(root,needtoempty))